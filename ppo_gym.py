...
from models.my_policy import MyPolicy
...
for i in range(args.num_threads):
    env.append(Underwater_navigation(args.depth_prediction_model,
               args.adaptation, args.randomization, i, args.hist_length))
...
def update_params(batch, i_iter):
    ...
    hist_actions = torch.from_numpy(
        np.stack(batch.hist_action)).to(dtype).to(device)
    ...
    for i_iter in range(args.max_iter_num):
        ...
        if args.eval_batch_size > 0:
            _, log_eval = agent.collect_samples(
                args.eval_batch_size, mean_action=True)
        t2 = time.time()
        ...
        if args.randomization == 1:
            if args.adaptation == 1:
                my_open = open(os.path.join(
                    assets_dir(), 'learned_models/{}_ppo_adapt.txt'.format(args.env_name)), "a")
            else:
                my_open = open(os.path.join(
                    assets_dir(), 'learned_models/{}_ppo_rand.txt'.format(args.env_name)), "a")
        else:
            my_open = open(os.path.join(
                assets_dir(), 'learned_models/{}_ppo_norand.txt'.format(args.env_name)), "a")

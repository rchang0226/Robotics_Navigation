...
# set clip to be 30 which is the maximum value for the depth value
...
"""create agent"""
agent = Agent(
    env, policy_net, device, running_state=running_state, num_threads=args.num_threads
)
...
def update_params(batch, i_iter):
    ...
    with torch.no_grad():
        ...
        fixed_log_probs = policy_net.get_log_prob(
            color_img, imgs_depth, goals, rays, hist_actions, actions
        )
    ...
    for _ in range(optim_epochs):
        ...
        (
            ...
            imgs_depth[perm].clone(),
            goals[perm].clone(),
            rays[perm].clone(),
            hist_actions[perm].clone(),
            actions[perm].clone(),
            returns[perm].clone(),
        ...
        for i in range(optim_iter_num):
            ind = slice(
                i * optim_batch_size,
                min((i + 1) * optim_batch_size, imgs_depth.shape[0]),
            )
            (
                color_img_b,
                imgs_depth_b,
        ...
        if args.randomization == 1:
            if args.adaptation == 1:
                my_open = open(
                    os.path.join(
                        assets_dir(),
                        "learned_models/{}_ppo_adapt.txt".format(args.env_name),
                    ),
                    "a",
                )
            else:
                my_open = open(
                    os.path.join(
                        assets_dir(),
                        "learned_models/{}_ppo_rand.txt".format(args.env_name),
                    ),
                    "a",
                )
        else:
            my_open = open(
                os.path.join(
                    assets_dir(),
                    "learned_models/{}_ppo_norand.txt".format(args.env_name),
                ),
                "a",
            )
        data = [
            ...
            str(log["avg_reward"]),
            " ",
            str(log["num_episodes"]),
            " ",
            str(log["ratio_success"]),
            " ",
            str(log["avg_steps_success"]),
            " ",
            str(log["avg_last_reward"]),
            "\n",
        ]
        for element in data:
            ...
            if args.randomization == 1:
                if args.adaptation == 1:
                    pickle.dump(
                        ...
                        open(
                            os.path.join(
                                assets_dir(),
                                "learned_models/{}_ppo_adapt.p".format(args.env_name),
                            ),
                            "wb",
                        ),
                    )
                    ...
                    pickle.dump(
                        ...
                        open(
                            os.path.join(
                                assets_dir(),
                                "learned_models/{}_ppo_rand.p".format(args.env_name),
                            ),
                            "wb",
                        ),
                    )
                ...
                pickle.dump(
                    ...
                    open(
                        os.path.join(
                            assets_dir(),
                            "learned_models/{}_ppo_norand.p".format(args.env_name),
                        ),
                        "wb",
                    ),
                )

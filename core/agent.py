...
def collect_samples(
        ...
        for t in range(10000):
            ...
            if running_state is not None:
                ...
                next_img_depth, next_goal, next_ray, next_hist_action = (
                    next_img_depth.astype(np.float64),
                    next_goal.astype(np.float64),
                    next_ray.astype(np.float64),
                    next_hist_action.astype(np.float64),
                )
    ...
    if num_episodes_success != 0:
        log["avg_steps_success"] = float(num_steps_episodes) / float(
            num_episodes_success
        )
    else:

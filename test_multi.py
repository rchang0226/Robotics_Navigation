...
def create_env(rank):
    ...
    unity_env = UnityEnvironment(
        "/home/pengzhi1998/navigation/mega_navigation/Robotics_Navigation/underwater_env/water",
        side_channels=[config_channel, pos_info],
        worker_id=rank,
        base_port=5000 + rank,
    )
...
class Agent:
    def __init__(
        self, env, policy, device, custom_reward=None, running_state=None, num_threads=2
    ):
        self.env = env
        self.policy = policy
        self.device = device
...
if __name__ == "__main__":
    ...
    agent = Agent(
        env, policy_net, device, custom_reward=None, running_state=None, num_threads=2
    )
